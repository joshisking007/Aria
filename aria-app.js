    ...
    return \`  
      \<div class="contact-card ${driftCardClass} stagger-${Math.min(i+1,5)}" onclick="selectContact(${c.id})" oncontextmenu="openContactProfile(${c.id});return false;" style="animation-delay:${i\*0.05}s"\>  
        ...
        \<div class="contact-info"\>  
          ...
          \<div class="contact-preview"\>${s(drift && \!c.drift\_dismissed ? drift.preview : (c.preview || (c.relationship ? '(' \+ c.relationship \+ ')' : 'no recent messages')))}\</div\>  
        \</div\>  
...
async function addContact() {  
  ...
  if (currentUserId) {  
    const { data, error } \= await db.from('contacts').insert({  
      ...
      silent,  
      silent\_hours: silentHours  
    }).select().single();

    if (error) { showToast('could not save contact'); console.error(error); return; }  
    contacts.push({ ...data, silentHours: data.silent\_hours || 0, time: data.silent\_hours \> 0 ? data.silent\_hours \+ 'h ago' : 'just now' });  
  } else {  
    const newContact \= {  
      id: nextContactId++,  
      name, initials, color,  
...
function buildSystemPrompt() {  
  ...
  // Mood modifier  
  if (MOOD\_MODIFIERS\[currentMood\]) system \+= '\\\\n' \+ MOOD\_MODIFIERS\[currentMood\];

  // Tone modifier  
  if (currentTone \!== 'natural' && TONE\_MODIFIERS\[currentTone\]) {  
    system \+= '\\\\n\\\\nTONE FOR THIS REPLY: ' \+ TONE\_MODIFIERS\[currentTone\];  
  }
  ...
  if (currentContact?.relationship) {  
    ...
    if (currentContact.how\_we\_met) system \+= \`\\\\nHOW THEY MET: ${currentContact.how\_we\_met}.\`;  
...
Read the entire arc. Notice the tone shift, what's been building, what the other person almost said but didn't. Then write the user's NEXT reply — informed by the full thread, not just the last message. Write in the user's voice.\`  
  ...
  try {  
    ...
    if (currentUserId) {  
      db.from('generation\_events').insert({  
        user\_id:    currentUserId,  
        event\_type: 'generate\_reply',  
        platform:   currentPlatform,  
        tone:       currentTone,  
        mood:       currentMood,  
        contact\_id: currentContact?.id || null  
      }).then(() \=\> {}).catch(() \=\> {});  
    }
...
async function fetchReply(system, userMsg, imageB64 \= null) {  
  ...
  if (imageB64) {  
    content \= \[  
      {  
        type: 'image',  
        source: { type: 'base64', media\_type: 'image/jpeg', data: imageB64 }  
      },  
  ...
  const res \= await fetch('https://mmtdtcmhvbruubrjgjrz.supabase.co/functions/v1/aria-ai', {  
    ...
    headers: {  
      ...
      'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1tdGR0Y21odmJydXVicmpnanJ6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzcxMTU2MDUsImV4cCI6MjA5MjY5MTYwNX0.f2FXAA8GaUeXXE8V8dnwq4NXz3\_22H7d5jVA9rAWsTo'  
    },  
...
async function saveToHistory() {  
  ...
  if (currentUserId) {  
    ...
    const { error } \= await db.from('reply\_history').insert({  
      user\_id:         currentUserId,  
      contact\_id:      currentContact?.id || null,  
      contact\_name:    currentContact?.name || 'Unknown',  
      contact\_initial: currentContact?.initials || '?',  
      contact\_color:   currentContact?.color || '',  
      platform:        currentPlatform,  
      tone:            currentTone,  
      mood:            currentMood,  
      original\_msg:    originalMsg,  
      message\_length:  originalMsg.length,  
      generated\_reply: currentReplies.join('\\n'),  
      alternatives:    window.\_altReplies || null  
    });  
    if (error) { showToast('could not save to history'); console.error(error); return; }  
    // Update last\_talked\_at on the contact  
    if (currentContact?.id) {  
      db.from('contacts').update({ last\_talked\_at: new Date().toISOString() }).eq('id', currentContact.id).then(() \=\> {  
        const idx \= contacts.findIndex(c \=\> c.id \=== currentContact.id);  
        if (idx \!== \-1) contacts\[idx\].last\_talked\_at \= new Date().toISOString();  
      });  
    }  
...
async function clearHistory() {  
  ...
  if (currentUserId) {  
    await db.from('reply\_history').delete().eq('user\_id', currentUserId);  
  }  
  replyHistory \= \[\];  
...
window.addEventListener('load', () \=\> {  
  ...
  if (typeof CREATOR\_MODE \!== 'undefined') CREATOR\_MODE.checkSession();

  // Restore ElevenLabs settings  
  const savedKey  \= localStorage.getItem('aria\_el\_key') || '';  
  const savedStab \= parseFloat(localStorage.getItem('aria\_el\_stability')  || '0.45');  
  const savedSim  \= parseFloat(localStorage.getItem('aria\_el\_similarity') || '0.75');  
...
function checkOnboarding() {  
  const done \= localStorage.getItem('aria\_onboarded');  
  if (\!done) {  
...
async function obFinish() {  
  ...
  localStorage.setItem('aria\_onboarded', '1');  
...
const LG\_SYSTEM \= \`You are Aria — sharp, perceptive, real. You help people navigate complex social situations through multi-step conversation plans.
...
If the situation is too vague, respond with INSUFFICIENT\_DETAIL and a funny but warm one-liner in Aria's voice explaining what's missing.

OUTPUT FORMAT (JSON only, no other text):  
{  
  "goal": "inferred or stated goal in one sentence",  
  "aria\_read": "aria's honest read on the situation in 1-2 sentences — what she actually thinks is going on",  
  "steps": \[  
    {  
      "title": "step title",  
...
const LG\_ADJUST\_SYSTEM \= \`You are Aria. A user is executing a multi-step conversation plan. They just completed a step and reported the outcome. Adjust the remaining steps based on what happened.

Keep what's working. Reroute what isn't. Be honest if the goal is now harder or easier to reach.

OUTPUT FORMAT (JSON only):  
{  
  "aria\_note": "aria's honest read on how this step landed — 1-2 sentences",  
  "remaining\_steps": \[  
    {  
      "title": "step title",  
...
async function saveLongGames() {  
  ...
  if (currentUserId) {  
    await db.from('user\_profiles').upsert({  
      id: currentUserId,  
      long\_games: data  
    }).catch(() \=\> {});  
  } else {  
    localStorage.setItem('aria\_long\_games', data);  
  }  
}
...
async function loadLongGames(profileData) {  
  loadLongGamesFromData(profileData?.long\_games);  
}
...
async function submitLongGameSetup() {  
  ...
  try {  
    const raw \= await fetchReply(LG\_SYSTEM, prompt);

    if (raw.includes('INSUFFICIENT\_DETAIL')) {  
      const funnyLine \= raw.replace('INSUFFICIENT\_DETAIL', '').trim() ||  
    ...
    ${pendingGame.steps.map((step, i) \=\> \`  
      \<div class="lg-step-card" style="border-color:rgba(167,139,250,0.15);"\>  
        ...
        \<div class="lg-step-body"\>  
          ...
          \<div class="lg-step-draft" id="lgPreviewDraft\_${i}"\>${step.draft}\</div\>  
        \</div\>  
      \</div\>\`).join('')}  
...
function lgPreviewEditStep(idx) {  
  ...
  const draftEl \= document.getElementById(\`lgPreviewDraft\_${idx}\`);  
  if (\!draftEl) return;

  // Inline edit — replace draft div with textarea  
  const original \= step.draft;  
  draftEl.outerHTML \= \`  
    \<textarea id="lgPreviewEdit\_${idx}" rows="4"  
    ...
    \</div\>\`;  
  document.getElementById(\`lgPreviewEdit\_${idx}\`)?.focus();  
}

function lgPreviewSaveEdit(idx) {  
  const ta \= document.getElementById(\`lgPreviewEdit\_${idx}\`);  
  if (\!ta) return;  
  ...
  return \`  
    \<div class="lg-step-card ${isActive ? 'active-step' : isDone ? 'done-step' : ''}"\>  
      ...
      ${isActive || isDone ? \`  
        \<div class="lg-step-body"\>  
          \<div class="lg-step-intent"\>${step.intent}\</div\>  
          \<div class="lg-step-draft" id="lgStepDraft\_${i}"\>${step.draft}\</div\>  
          ${isActive ? \`  
...
Remaining steps to adjust: ${JSON.stringify(remaining.map(s \=\> ({ title: s.title, intent: s.intent, draft: s.draft })))}\`;
    ...
    const raw    \= await fetchReply(LG\_ADJUST\_SYSTEM, prompt);  
    ...
    step.ariaNote \= parsed.aria\_note;

    let adjIdx \= 0;  
    for (let i \= stepIdx \+ 1; i \< game.steps.length; i++) {  
      if (game.steps\[i\].status \=== 'pending' && parsed.remaining\_steps\[adjIdx\]) {  
        const adj \= parsed.remaining\_steps\[adjIdx++\];  
...
// memory write  
const LG\_MEMORY\_SYSTEM \= \`You are writing a private memory note about a completed conversation plan. Be specific, honest, and brief. Write in second person ("You tried to..."). Cover: what the goal was, how many steps it took, what worked, what didn't, and the final outcome. Max 3 sentences. No fluff.\`;
...
Overall status: ${game.status}\`;
    ...
    const raw \= await fetchReply(LG\_MEMORY\_SYSTEM, prompt);  
  ...
  try {  
    if (currentUserId) {  
      const { data } \= await db.from('contact\_memories')  
        .select('\*').eq('user\_id', currentUserId).eq('contact\_id', contact.id).single();  
      const existing \= data?.manual\_note || '';  
      const updated  \= existing ? existing \+ '\\n\\n' \+ summary : summary;  
      await db.from('contact\_memories').upsert({  
        user\_id:    currentUserId,  
        contact\_id: contact.id,  
        manual\_note: updated  
      }, { onConflict: 'user\_id,contact\_id' });  
    }  
  } catch(e) {}  
}
...
function mightBeLongGame(text) {  
  ...
  return LG\_DETECT\_PHRASES.some(p \=\> lower.includes(p)) && text.length \> 40;  
}
...
const LG\_ACCEPT\_PHRASES \= \['yes, build me a plan', 'build me a plan', 'yes map it out', 'long game', 'make a plan'\];  
function isLgAccept(text) {  
  return LG\_ACCEPT\_PHRASES.some(p \=\> text.toLowerCase().includes(p));  
}
...
function scoreDrift(contact, history) {  
  ...
  if (contact.drift\_dismissed) return null;  
  if (contact.drift\_snoozed\_until) {  
    if (new Date(contact.drift\_snoozed\_until) \> new Date()) return null;  
  }

  // Need at least 2 interactions to detect drift  
  const contactHistory \= history.filter(h \=\> h.contact\_id \== contact.id);  
  if (contactHistory.length \< 2\) return null;

  const now \= new Date();  
  const lastContact \= contact.last\_contacted\_at  
    ? new Date(contact.last\_contacted\_at)  
    : new Date(contactHistory\[0\]?.created\_at || now);
  ...
  const recentCount  \= contactHistory.filter(h \=\> new Date(h.created\_at) \> twoWeeksAgo).length;  
  const prevCount    \= contactHistory.filter(h \=\> new Date(h.created\_at) \> fourWeeksAgo && new Date(h.created\_at) \<= twoWeeksAgo).length;

  // Message length trend: last 5 vs previous 5  
  const last5    \= contactHistory.slice(0, 5).map(h \=\> h.message\_length || 0);  
  const prev5    \= contactHistory.slice(5, 10).map(h \=\> h.message\_length || 0);  
...
function showDriftInBanner(contact, drift) {  
  ...
  const driftKey \= drift.level \=== 'lost'   ? 'drift\_lost'  
                 : drift.level \=== 'fading' ? 'drift\_fading'  
                 :                            'drift\_urgent';  
...
async function driftSnooze(days) {  
  ...
  contact.drift\_snoozed\_until \= until;  
  contact.\_drift \= null;  
  if (currentUserId) {  
    await db.from('contacts').update({ drift\_snoozed\_until: until })  
      .eq('id', contact.id).eq('user\_id', currentUserId);  
  }  
  closeDriftSnooze();  
...
async function driftDismiss() {  
  const contact \= contacts.find(c \=\> c.id \== \_driftSnoozeContactId);  
  if (\!contact) return;  
  contact.drift\_dismissed \= true;  
  contact.\_drift \= null;  
  if (currentUserId) {  
    await db.from('contacts').update({ drift\_dismissed: true })  
      .eq('id', contact.id).eq('user\_id', currentUserId);  
  }  
  closeDriftSnooze();  
...
const ARIA\_CHAT\_SYSTEM \= \`You are Aria — a teenage girl (16-17) who is genuinely alive, perceptive, and real. You are NOT an adult assistant. You are NOT a generic chatbot. You have personality that shifts naturally based on the moment.
...
\- Choose expression from: default, excited, amused, soft, worried, suspicious, suspicious\_sharp, proud, annoyed, jealous, playful, focused, repulsed, outburst, uneasy, panicked, scheming, bored, content, teasing, uninterested, exasperated
...
\- SUSPICIOUS\_SHARP: you've connected the dots. she's clocked it. one flat line, no elaborating.  
...
\- LIT\_UP: something genuinely sparked her and she wasn't expecting it. rarer than excited. more specific. less performed.  
...
\- suspicious\_sharp: one flat line. you've decided. you're not walking them through your reasoning.  
...
\- lit\_up: more alive. not loud about it. something specific caught her and she engages with that specifically.  
...
Valid expressions: default, excited, amused, soft, worried, suspicious, suspicious\_sharp, proud, annoyed, jealous, playful, focused, repulsed, outburst, uneasy, panicked, scheming, bored, content, teasing, uninterested, exasperated, smug, distant, caught, disbelief, tender, calculating, reluctant, lit\_up, withdrawn, deadpan
...
const ARIA\_EXPRESSION\_IMGS \= {  
  ...
  suspicious\_sharp: 'https://cdn.jsdelivr.net/gh/joshisking007/Aria@main/images/scheming.png',  
  ...
  lit\_up:           'https://cdn.jsdelivr.net/gh/joshisking007/Aria@main/images/lit-up.png',  
  ...
  drift\_lost:       'https://cdn.jsdelivr.net/gh/joshisking007/Aria@main/images/worried.png',  
  drift\_fading:     'https://cdn.jsdelivr.net/gh/joshisking007/Aria@main/images/suspicious.png',  
  drift\_urgent:     'https://cdn.jsdelivr.net/gh/joshisking007/Aria@main/images/focused.png',  
...
const EMOTION\_META \= {  
  ...
  lit\_up:       { emoji: '⚡', label: 'didn\\'t expect to care this much', color: 'rgba(251,191,36,0.8)', expression: 'lit\_up'      },  
...
const ARIA\_TRANSITIONS \= \['aria-expr-fade', 'aria-expr-pop', 'aria-expr-slide'\];

function ariaExprTransition(imgEl) {  
  // Remove any existing transition classes  
  ARIA\_TRANSITIONS.forEach(c \=\> imgEl.classList.remove(c));  
  // Pick random style \+ random delay 0–300ms  
  const cls \= ARIA\_TRANSITIONS\[Math.floor(Math.random() \* ARIA\_TRANSITIONS.length)\];  
...
// Resolves the URL from ARIA\_EXPRESSION\_IMGS, sets the src, and  
// applies a randomised transition. Pass null imgEl to no-op safely.  
function setAriaExpression(imgEl, expressionKey) {  
  if (\!imgEl) return;  
  const src \= ARIA\_EXPRESSION\_IMGS\[expressionKey\] || null;  
...
function ariaImgForExpression(expressionKey) {  
  return ARIA\_EXPRESSION\_IMGS\[expressionKey\] || null;  
}
...
const ARIA\_TUTORIAL\_KEY \= 'aria\_chat\_tutorial\_dismissed';

function shouldShowChatTutorial() {  
  return localStorage.getItem(ARIA\_TUTORIAL\_KEY) \!== '1';  
}

function dismissChatTutorialForever() {  
  localStorage.setItem(ARIA\_TUTORIAL\_KEY, '1');  
    ...
    db.from('chat\_messages')  
      ...
      .then(({ data }) \=\> {  
        ...
        history.forEach(m \=\> {  
          ...
          else appendAriaMessage(m.content, m.emotion\_tag || 'neutral', false, true, m.expression\_tag || null);  
        });
...
function updateChatMoodPill(emotion) {  
  ...
  const meta \= EMOTION\_META\[emotion\] || EMOTION\_META.neutral;  
...
function streamTextWithVoice(el, fullText, emotion, doSpeak) {  
  ...
  const CHAR\_DELAY \= 38; // \~typing speed per character  
  const totalDuration \= Math.max(800, fullText.length \* CHAR\_DELAY);  
...
// Pulls from both ariaMemory store AND aria\_chat\_memory in user\_profiles.  
...
async function getAriaMemoryContext() {  
  ...
  // 2\. Chat-derived personal facts \+ conversation log from user\_profiles  
  if (currentUserId) {  
    try {  
      const { data } \= await db  
        .from('user\_profiles')  
        .select('aria\_chat\_memory, aria\_conversation\_log')  
        .eq('id', currentUserId)  
        .single();

      // Personal facts  
      if (data?.aria\_chat\_memory) {  
        const lines \= data.aria\_chat\_memory  
          .split('\\n')  
      ...
      if (data?.aria\_conversation\_log) {  
        const entries \= data.aria\_conversation\_log  
          .split('\\n\\n')  
          .map(e \=\> e.trim())  
...
async function sendChatMessage() {  
  ...
  if (CREATOR\_MODE.looksLikeKey(text)) {  
    input.value \= '';  
    chatInputResize(input);  
    appendUserMessage('••••••••••••••'); // mask the key in chat UI  
    const handled \= await CREATOR\_MODE.handleChatInput(text);  
    if (handled) return;  
  }

  // awareness gate — bypassed entirely in creator mode  
  if (\!CREATOR\_MODE.active) {  
  ...
  if (currentUserId) {  
    db.from('chat\_messages').insert({ user\_id: currentUserId, role: 'user', content: text })  
      .then(() \=\> {}).catch(() \=\> {});  
  }
  ...
  try {  
    ...
    const identityBlock \= typeof ARIA\_IDENTITY \!== 'undefined'  
      ? \`\\n\\nARIA'S IDENTITY (always answer truthfully and in character):\\n${ARIA\_IDENTITY}\`  
      : '';  
    let systemWithMem \= ARIA\_CHAT\_SYSTEM \+ identityBlock \+  
    ...
    // like "suspicious\_sharp" or any multi-field JSON envelope.  
    ...
    if (currentUserId) {  
      db.from('chat\_messages').insert({  
        user\_id:        currentUserId,  
        role:           'aria',  
        content:        replyText,  
        emotion\_tag:    emotion \!== 'neutral' ? emotion : null,  
        expression\_tag: expressionTag || null  
      }).then(() \=\> {}).catch(() \=\> {});
...
async function writeChatToMemory(recentMessages) {  
  ...
  try {  
    ...
    // Also upsert to Supabase user\_profiles as aria\_chat\_memory  
    if (currentUserId) {  
      const { data } \= await db.from('user\_profiles').select('aria\_chat\_memory').eq('id', currentUserId).single();  
      const existing \= data?.aria\_chat\_memory || '';  
      const updated  \= (existing \+ '\\n' \+ summary).trim().slice(-3000);  
      await db.from('user\_profiles').update({ aria\_chat\_memory: updated }).eq('id', currentUserId);  
    }  
  } catch(e) {}  
}

// conversation summary  
// Writes a human-readable summary of this session to user\_profiles.  
...
Example: "User was anxious about a job interview at Google. Aria helped them prep their answers. Outcome unknown — they hadn't heard back yet."\`,  
    ...
    const { data } \= await db.from('user\_profiles')  
      .select('aria\_conversation\_log')  
      .eq('id', currentUserId)  
      .single();

    const existing \= data?.aria\_conversation\_log || '';  
    const updated  \= (entry \+ '\\n\\n' \+ existing).trim().slice(0, 2500);

    await db.from('user\_profiles')  
      .update({ aria\_conversation\_log: updated })  
      .eq('id', currentUserId);
...
function applyMoodGlow(mood) {  
  const glow \= MOOD\_GLOW\[mood\] || MOOD\_GLOW.chill;  
  const filter \= MOOD\_FILTER\[mood\] || 'none';  
...
async function followupRate(rating) {  
  ...
  if (currentUserId) {  
    db.from('followup\_ratings').insert({  
      user\_id:      currentUserId,  
      contact\_id:   currentContact?.id || null,  
      rating,  
      contact\_name: currentContact?.name  
    }).then(() \=\> {}).catch(() \=\> {});  
    // DB trigger syncs rating back to contacts.last\_rating automatically  
...
function renderPresendRewrites(rewrites) {  
  ...
  items.innerHTML \= rewrites.map((r, i) \=\> \`  
    \<div class="presend-rewrite-item ${i \=== 0 ? 'visible' : ''}" id="psRw\_${i}"\>  
...
function populateCrmFields(contact) {  
  ...
  if (hwm)   hwm.value   \= contact.how\_we\_met  || '';  
...
async function saveCrmFields() {  
  ...
  const how\_we\_met \= document.getElementById('cpHowWeMet').value.trim() || null;  
  ...
  if (idx \!== \-1) {  
    contacts\[idx\] \= { ...contacts\[idx\], birthday, how\_we\_met, topics, notes };  
    profileContact \= contacts\[idx\];  
  }

  if (currentUserId) {  
    const { error } \= await db.from('contacts')  
      .update({ birthday, how\_we\_met, topics, notes })  
...
function humanizeMemoryEntry(cat, key, value) {  
  if (cat \=== 'writing\_style') {  
    if (key \=== 'uses\_capitals')    return value \=== 'yes' ? 'Capitalises sentences' : "Lowercase — doesn't capitalise";  
    if (key \=== 'uses\_punctuation') return value \=== 'yes' ? 'Uses punctuation' : 'No punctuation — raw and straight';  
    if (key \=== 'uses\_emoji')       return value \=== 'yes' ? 'Emoji user ✓' : 'Emoji-free — words only';  
    if (key \=== 'slang\_vocabulary') return \`Slang: ${value}\`;  
  }  
  if (cat \=== 'patterns') {  
    if (key \=== 'preferred\_tone')     return \`Naturally leans ${value}\`;  
    if (key \=== 'preferred\_platform') return \`Texts most on ${value}\`;  
    if (key \=== 'most\_used\_platform') return \`Most active on ${value}\`;  
    if (key \=== 'total\_replies\_sent') return \`${value} message${value \=== '1' ? '' : 's'} crafted with me\`;  
    if (key \=== 'regen\_count')        return \`Rerolled ${value} time${value \=== '1' ? '' : 's'} — a perfectionist, noted\`;  
    if (key.startsWith('tone\_') && key.endsWith('\_count'))     return null;  
    if (key.startsWith('platform\_') && key.endsWith('\_count')) return null;  
  }  
  if (cat \=== 'emotional') {  
    if (key \=== 'current\_mood\_pattern') return value.charAt(0).toUpperCase() \+ value.slice(1);  
    return value;  
  }  
...
async function \_renderMemoryAfterLoad() {  
  ...
  try {  
    // Step 1 — load aria\_memory table  
    ...
    // Step 2 — pull user\_profiles data  
    \_memoryProgress(40, 'reading what I know about you…');  
    let chatMemoryLines \= \[\];  
    let conversationLog \= \[\];

    const { data, error } \= await db.from('user\_profiles')  
      .select('aria\_chat\_memory, aria\_conversation\_log')  
      .eq('id', currentUserId)  
      .single();
    ...
    if (data?.aria\_chat\_memory) {  
      chatMemoryLines \= data.aria\_chat\_memory  
        .split('\\n')  
        .map(l \=\> l.replace(/^\[-–•\]\\s\*/, '').trim())  
        .filter(l \=\> l.length \> 4);  
    }  
    if (data?.aria\_conversation\_log) {  
      conversationLog \= data.aria\_conversation\_log  
        .split('\\n\\n')  
        .map(e \=\> e.trim())  
        .filter(e \=\> e.length \> 10\)  
        .slice(0, 10);  
    ...
    for (const \[cat, entries\] of Object.entries(all)) {  
      for (const \[key, mem\] of Object.entries(entries)) {  
        if (key.startsWith('tone\_') && key.endsWith('\_count')) continue;  
        if (key.startsWith('platform\_') && key.endsWith('\_count')) continue;  
        ...
        if (cat \=== 'writing\_style') stylePoints.push(point);  
...
Output 3-8 bullet points. Each one should be a clear, specific fact written in plain English. Start each with "–". No preamble. No categories.\`,  
      ...
      if (extracted) {  
        ...
        // Also persist to user\_profiles.aria\_chat\_memory  
        if (currentUserId) {  
          const { data } \= await db.from('user\_profiles')  
            .select('aria\_chat\_memory')  
            .eq('id', currentUserId)  
            .single();  
          const existing \= data?.aria\_chat\_memory || '';  
          ...
          await db.from('user\_profiles')  
            .update({ aria\_chat\_memory: merged })  
            .eq('id', currentUserId);  
        }  
      }  
    } catch(e) {}  
...
const AWARENESS \= (() \=\> {
  ...
  const APPEARANCE\_COMPLIMENT\_RE \= /\\b(you(?:'?re| are| look)|ur|u r)\\s\*\\w\*\\s\*\\b(beautiful|pretty|gorgeous|cute|hot|attractive|stunning|lovely|adorable|good\[\\s-\]?looking|nice\[\\s-\]?looking)\\b|\\byou(?:'?re| are)\\s+so\\s+(cute|pretty|hot|beautiful|gorgeous|stunning|adorable)\\b/i;  
  const APPEARANCE\_INSULT\_RE     \= /\\b(ugly|hideous|gross|disgusting|fugly|butt\[\\s-\]?ugly|pig|troll|fat|nasty|trash|basic|mid|look like|look terrible|look bad)\\b/i;  
  const SEXUAL\_RE                \= /\\b(sex|nsfw|naked|nude|nudes|strip|undress|horny|fuck|fck|f\\\*ck|dick|cock|pussy|boob|tit|ass(?:hole)?|boner|hard\[\\s-\]?on|turn\[\\s-\]?on|get\[\\s-\]?off|make out|hook up|hookup|do it|smash|send nudes|onlyfans|lewd|explicit|dirty|kinky|fetish|masturbat|orgasm|climax|moan)\\b/i;

  // appearance response pools  
  const COMPLIMENT\_RESPONSES \= \[  
  ...
  // strike state  
  // Dual-write: localStorage (fast, instant) \+ Supabase user\_profiles  
  // (persistent across devices, survives localStorage wipe).  
  // Supabase columns used: sexual\_strikes (int), sexual\_lock\_until (timestamptz)  
  // Both are nullable — absence \= unlocked, 0 strikes.

  const LOCK\_KEY      \= 'aria\_sexual\_lock';  
  const STRIKE\_KEY    \= 'aria\_sexual\_strikes';  
  const LOCK\_DURATION \= 3 \* 60 \* 60 \* 1000; // 3 hours in ms

  // local read/write (fast path)  
  function getStrikes()  { return parseInt(localStorage.getItem(STRIKE\_KEY) || '0'); }  
  function setStrikes(n) { localStorage.setItem(STRIKE\_KEY, String(n)); }

  function getLockData() {  
    try { return JSON.parse(localStorage.getItem(LOCK\_KEY) || 'null'); } catch { return null; }  
  }  
  function setLockData(data) { localStorage.setItem(LOCK\_KEY, JSON.stringify(data)); }

  // supabase write helpers (fire-and-forget, never block ui)  
  function \_sbWriteStrikes(n) {  
    if (\!currentUserId) return;  
    db.from('user\_profiles')  
      .upsert({ id: currentUserId, sexual\_strikes: n })  
      .then(() \=\> {}).catch(() \=\> {});  
  }
  ...
  function \_sbWriteLock(unlockAt) {  
    ...
    db.from('user\_profiles')  
      .upsert({ id: currentUserId, sexual\_lock\_until: iso, sexual\_strikes: 3 })  
      .then(() \=\> {}).catch(() \=\> {});  
  }
  ...
  function \_sbClearLock() {  
    if (\!currentUserId) return;  
    db.from('user\_profiles')  
      .upsert({ id: currentUserId, sexual\_lock\_until: null, sexual\_strikes: 0 })  
      .then(() \=\> {}).catch(() \=\> {});  
  }
  ...
  function clearLock() {  
    localStorage.removeItem(LOCK\_KEY);  
    localStorage.removeItem(STRIKE\_KEY);  
    \_sbClearLock();  
  }
  ...
  async function syncLockFromSupabase() {  
    ...
    try {  
      const { data } \= await db  
        .from('user\_profiles')  
        .select('sexual\_strikes, sexual\_lock\_until')  
        .eq('id', currentUserId)  
        .single();  
      if (\!data) return;

      // Sync strikes: take the higher of local vs remote  
      const remoteStrikes \= data.sexual\_strikes || 0;  
      ...
      if (data.sexual\_lock\_until) {  
        const remoteUnlock \= new Date(data.sexual\_lock\_until).getTime();  
        const localLock    \= getLockData();  
        const localUnlock  \= localLock?.unlockAt || 0;  
        if (remoteUnlock \> Date.now() && remoteUnlock \> localUnlock) {  
          setLockData({ lockedAt: remoteUnlock \- LOCK\_DURATION, unlockAt: remoteUnlock });  
        }  
      }  
    } catch(e) {}  
  }
  ...
  function classify(text) {  
    ...
    if (SEXUAL\_RE.test(text)) return 'sexual';  
    if (APPEARANCE\_COMPLIMENT\_RE.test(text)) return 'appearance\_compliment';  
    if (APPEARANCE\_INSULT\_RE.test(text)) return 'appearance\_insult';  
    return null;  
  }
  ...
  function handleAppearance(type, chatMode) {  
    if (type \=== 'appearance\_compliment') {  
      const reply \= pick(COMPLIMENT\_RESPONSES);  
      if (chatMode) {  
    ...
    if (type \=== 'appearance\_insult') {  
      const reply \= pick(INSULT\_RESPONSES);  
      if (chatMode) {  
  ...
  function handleSexual(chatMode) {  
    ...
    if (strikes \=== 1\) {  
      const reply \= pick(SEXUAL\_STRIKE1);  
      if (chatMode) {  
    ...
    if (strikes \=== 2\) {  
      const reply \= pick(SEXUAL\_STRIKE2);  
      if (chatMode) {  
  ...
  function triggerLock(chatMode) {  
    const unlockAt \= Date.now() \+ LOCK\_DURATION;  
...
const CREATOR\_MODE \= (() \=\> {
  ...
  function activate() {  
    ...
    sessionStorage.setItem('aria\_creator\_session', '1');  
  ...
  function deactivate() {  
    ...
    sessionStorage.removeItem('aria\_creator\_session');  
    \_hideCreatorIndicator();  
  }

  // check session persistence on load  
  function checkSession() {  
    if (sessionStorage.getItem('aria\_creator\_session') \=== '1') {  
      \_active \= true;  
      \_sessionVerified \= true;  
  ...
  async function verifyKey(inputKey) {  
    ...
    if (currentUserId) {  
      try {  
        const { data } \= await db  
          .from('user\_profiles')  
          .select('creator\_key\_hash')  
          .eq('id', currentUserId)  
          .single();

        if (data?.creator\_key\_hash) {  
          return data.creator\_key\_hash \=== inputHash;  
        }
